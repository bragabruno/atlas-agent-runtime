# atlas-agent-runtime — build system (single source of truth).
...
test: ## Offline unit tests (pytest; injected fakes, zero API spend)
	@./scripts/test.sh

coverage: ## Coverage gate (pytest-cov; recommended, ATLAS_COV_MIN)
	@./scripts/coverage.sh

build: ## Build verification (import smoke — app package)
	@./scripts/build.sh
...
local: ## Run the service locally (trigger surface — ADR-020)
	@./scripts/local.sh
